...
if (-not (Get-Command choco -ErrorAction SilentlyContinue)) {
    ...
    # Reload env vars so we can use choco immediately
...
# 4. Create "Tools" Folder on Desktop for convenience
...
'@
Add-Type $code
# Set solid color (RGB for Red is "0 0 255" in Registry, but simple wallpaper removal works too for now)
# Setting background to solid red via registry for next login
Set-ItemProperty -Path "HKCU:\Control Panel\Colors" -Name "Background" -Value "255 0 0"
Set-ItemProperty -Path "HKCU:\Control Panel\Desktop" -Name "WallPaper" -Value ""
# Force update (might require logout to be fully visible as red)
